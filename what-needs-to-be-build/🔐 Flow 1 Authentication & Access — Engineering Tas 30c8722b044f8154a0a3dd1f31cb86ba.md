...
- [ ]  Valid email + password → redirects to `/org/[slug]/facilities`
- [ ]  Invalid password → shows "Email o contraseña incorrectos" (don't reveal which is wrong)
- [ ]  Non-existent email → shows same generic error (don't leak whether email exists)
- [ ]  Empty fields → show field-level validation before submitting
- [ ]  Email format validation (client-side)
- [ ]  Password minimum 8 characters (client-side)
- [ ]  "Remember me" checkbox extends session to 30 days (default 24h)
- [ ]  Loading state on button while request is in-flight
- [ ]  Disabled button during submission (prevent double-submit)
- [ ]  If user is already authenticated → redirect to `/org` immediately
...
- [ ]  Click "Continuar con Google" → opens Google OAuth flow
- [ ]  Google account with matching email in `users` table → login succeeds, redirect to `/org`
- [ ]  Google account with email NOT in `users` table → creates user, but has no org → dead-end page
...
- [ ]  `/register` without `?token` → show message: "Necesitas una invitación para crear una cuenta. Contacta al administrador de tu organización."
- [ ]  Valid token → email field pre-filled and disabled (read-only)
- [ ]  Token's organization name shown: "Has sido invitado a [Org Name]"
- [ ]  Role shown in invite context: "como Administrador" / "como Manager de [Facility Name]"
- [ ]  Display name field: required, 2-100 chars
- [ ]  Password field: required, min 8 chars
- [ ]  Confirm password field: must match password
- [ ]  On submit: user created, invite status set to `accepted`, `acceptedAt` timestamp set
- [ ]  User added to `organization_members` with role and facility_ids from invite
- [ ]  Auto-login after account creation (no need to go to `/login`)
- [ ]  Redirect to `/org/[orgSlug]/facilities` after creation
- [ ]  If user with that email already exists → show: "Ya existe una cuenta con este email. Inicia sesión para aceptar la invitación." with link to `/login`
...
- [ ]  Expired token (>7 days) → show: "Esta invitación ha expirado. Pide al administrador que envíe una nueva."
- [ ]  Already-used token (`status: accepted`) → show: "Esta invitación ya fue utilizada. Si ya tienes cuenta, inicia sesión."
