...
- [x] TASK-PB.05 — Create `publicBooking` tRPC router with read procedures: `getFacility`, `getAvailableSlots`, `calculatePrice`. All use `publicProcedure`. Write tests. Depends on TASK-PB.04.
- [x] TASK-PB.06 — Add OTP procedures to `publicBooking` router: `sendOtp`, `verifyOtp`. Integrate `@wifo/whatsapp`. Rate-limit with Upstash Redis. Write tests. Depends on TASK-PB.02, TASK-PB.05.
- [x] TASK-PB.07 — Add booking mutation procedures: `createBooking` (with OTP token validation + conflict detection + Turnstile), `getMyBookings`, `cancelBooking`. Reuse existing price calculation. Write tests. Depends on TASK-PB.06.
...
- [x] TASK-PB.10 — Build booking flow: `/[facilitySlug]/book` (court & time slot grid), `/[facilitySlug]/confirm` (name + phone + OTP verification), `/[facilitySlug]/success` (confirmation with booking code + "add to calendar"). Depends on TASK-PB.07, TASK-PB.09.
...
- [x] TASK-PB.12 — Send WhatsApp booking confirmation message via Kapso after successful booking: booking details, facility name, date/time, court, booking code. Depends on TASK-PB.02, TASK-PB.10.

#### Phase 4: Dashboard Integration

- [x] TASK-PB.13 — Add "Enlace de reservas" section to facility settings in dashboard: shareable URL display, QR code generator, copy-to-clipboard button. Depends on TASK-PB.08.
