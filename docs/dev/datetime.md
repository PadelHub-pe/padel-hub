...
Currently `timestamp without time zone`. They round-trip correctly only because the postgres-js session is Lima-pinned. **Future ticket**: migrate to `timestamptz` (Rule 1: real instants want `timestamptz`). Until then, do not bypass the session pin (no raw `psql` writes, no other DB tooling without the same session config).
