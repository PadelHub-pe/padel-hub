...
async function getPeakPeriods(
  ...
  // A slot is "peak" if it has a higher price than the court's base price
  // For simplicity, we'll return templates that have prices
  // This can be refined based on actual peak pricing logic
...
export const calendarRouter = {
  ...
  getWeekView: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const mondayStart = startOfWeek(input.weekStart, { weekStartsOn: 1 });
        ...
        ctx.db
          ...
          .where(
            and(
              ...
              // We can't use "ne" easily, so we'll filter in code
            ),
          ),
      ]);
