...
type Tab = "jugadores" | "propietarios";
...
export function HowItWorksTabs() {
  ...
  const isPlayers = activeTab === "jugadores";
  const steps = isPlayers ? PLAYER_STEPS : OWNER_STEPS;
      ...
      <div role="tabpanel" className="grid grid-cols-1 gap-8 md:grid-cols-3">
        {steps.map((step) => (
          <div key={step.number} className="text-center">
            ...
            <div
              className={cn(
                "mb-3 inline-flex h-7 w-7 items-center justify-center rounded-full text-sm font-bold text-white",
                isPlayers ? "bg-secondary" : "bg-primary",
              )}
            >
              {step.number}
            </div>
